...
the API service with Cloud Run `--set-secrets`. Secret values are injected from
Secret Manager, and region comes from `REGION` in `.env` (us-west1).
...
| Slack bot token | api | `SLACK_BOT_TOKEN_SECRET` | secret `slack-bot-token` |
