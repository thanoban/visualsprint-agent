...
export function LiveMetricsRow({ meeting }: { meeting: MeetingDetail }) {
  ...
  return (
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
      ...
      <MetricCard label="Visual events" value={String(metrics.visualEventsCount)} />
      <MetricCard label="Transcript segments" value={String(metrics.transcriptSegmentsCount)} />
      <MetricCard label="Memory matches" value={String(metrics.memoryMatchesCount)} />
      <MetricCard label="Capture chunks" value={String(metrics.captureChunksCount)} />
